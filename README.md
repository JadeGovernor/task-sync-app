...
- 今日页里 **公司 / 自律 / 循环任务 / 计划** 都能直接打勾；计划打勾＝完成归档（和计划页的「完成」按钮同一套逻辑，可在计划页点「恢复」撤销）。
- tab 上的数字：今日＝今天还没打勾的条数；**公司＝今天该做的条数（含逾期顺延）**，和公司页「今日」桶完全一致；
  计划＝进行中的计划条数；自律＝今天要勾还没勾的习惯数；习惯＝清单条数。
